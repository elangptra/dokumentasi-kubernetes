...
> ✅ Requirement diatas hanya untuk menjalankan cluster, penggunaan resource meningkat seiiring bertambahnya apps yang di deploy
...
| Port / Protokol | Protokol | Fungsi |
|---|---|---|
| `6443` | TCP | Kubernetes API Server — pintu utama komunikasi cluster |
| `2379-2380` | TCP | etcd server — database cluster Kubernetes |
| `10250-10259` | TCP | Kubelet API, kube-scheduler, kube-controller-manager, metrics |
| `3306` / `13306-53306` | TCP | MySQL / MariaDB standar dan port alternatif multi-instance |
| `4444` | TCP |  |
| `4567` | TCP/UDP |  |
| `4568` | TCP |  |
| `5432-5532` | TCP |  |
...
| `9094` | TCP |  |
| `8030` `8040` `8060` | TCP |  |
| `9010` `9020` `9030` `9050` `9060` | TCP |  |
| `4200` `4300` | TCP |  |
| `179` | TCP | BGP — digunakan Calico untuk routing antar node |
| `4443` | TCP | Metrics Server / webhook HTTPS |
| `4789` | UDP | VXLAN — overlay network untuk pod communication |
| `30000-32767` | TCP/UDP | Kubernetes NodePort range |
| `51820` `52821` | UDP | WireGuard VPN (opsional, digunakan Calico WireGuard encryption) |
| `53` | TCP/UDP | DNS — CoreDNS untuk resolusi nama di dalam cluster |
| `9153` | TCP | CoreDNS metrics |
| Protocol `4` | — | IPIP tunneling — digunakan Calico untuk cross-subnet pod traffic |

---

### Firewall Rules — Worker Node

Jalankan **hanya di setiap Worker Node**. Worker node menggunakan zone khusus (sesuaikan nama zone dengan yang aktif di server Anda — lihat catatan di bawah):

```bash
sudo firewall-cmd --zone=mariadb --add-port=10250-10259/tcp --permanent
sudo firewall-cmd --reload

sudo firewall-cmd --zone=mariadb \
  --add-port=179/tcp \
  --add-port=4789/udp \
  --add-port=5432-5532/tcp \
  --add-port=51820/udp \
...
sudo firewall-cmd --zone=mariadb --add-masquerade --permanent
sudo firewall-cmd --zone=mariadb --add-forward --permanent
sudo firewall-cmd --reload
```

**Penjelasan konfigurasi tambahan Worker Node:**

| Konfigurasi | Fungsi |
|---|---|
| `--add-masquerade` | Mengaktifkan NAT/masquerade agar pod dapat berkomunikasi keluar ke jaringan eksternal menggunakan IP node |
| `--add-forward` | Mengaktifkan IP forwarding di level zone firewalld, agar trafik antar interface (pod ↔ node ↔ jaringan) dapat diteruskan |

> **📝 Catatan zona firewall:**
> Nama zone `mariadb` di atas hanyalah contoh — sesuaikan dengan nama zone yang aktif di server Anda. Untuk melihat zone yang aktif, jalankan:
> ```bash
...
**Ringkasan konfigurasi firewall yang diterapkan:**

```
ports:      3306/tcp 4444/tcp 4567/tcp 4568/tcp 4567/udp 9400/tcp 9300/tcp 9443/tcp
            6379/tcp 9200/tcp 13306/tcp 23306/tcp 33306/tcp 43306/tcp 53306/tcp
            8030/tcp 9020/tcp 9030/tcp 9010/tcp 9060/tcp 8040/tcp 9050/tcp 8060/tcp
            4300/tcp 4200/tcp 10250-10259/tcp 179/tcp 4789/udp 5432-5532/tcp
            51820/udp 52821/udp 2379-2380/tcp 30000-32767/udp 4443/tcp 6443/tcp
            30000-32767/tcp 9094/tcp 53/udp 53/tcp 9153/tcp
protocols:  4
forward:    yes
masquerade: yes
```
...
>
> `ufw` tidak mengenal konsep "zone" dan "masquerade" secara langsung seperti firewalld. Untuk mengaktifkan masquerade/NAT, tambahkan rule POSTROUTING di `/etc/ufw/before.rules`. Untuk port-port utama:
>
> **Master Node:**
> ```bash
...
> sudo ufw allow 52821/udp
> sudo ufw reload
> ```
>
> Untuk mengaktifkan IP forwarding dan masquerade di ufw, pastikan `net/ipv4/ip_forward=1` ada di `/etc/ufw/sysctl.conf` dan tambahkan rule NAT di `/etc/ufw/before.rules`.
